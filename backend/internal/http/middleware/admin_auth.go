...
// PlatformAdminOnly ensures the requester is an internal platform operator.
// Hospital-local admins must not reach cross-hospital management surfaces.
func PlatformAdminOnly(pool *pgxpool.Pool) gin.HandlerFunc {
	return func(c *gin.Context) {
		hospitalIDStr := c.GetString(CtxHospitalID)
		userIDStr := c.GetString(CtxUserID)
		...
		if err := pool.QueryRow(c.Request.Context(), q, hospitalID, userID).Scan(&isPlatformAdmin); err != nil {
			c.AbortWithStatusJSON(http.StatusInternalServerError, gin.H{"error": "authorization check failed"})
			return
		}
